...
class DeleteProduct(QWidget):
  ...
  def initUI(self):
    ...
    self.setWindowIcon(QIcon("delete1.png"))
    ...
    self.btn_update.clicked.connect(self.on_delete_clicked)
...
def delete_data(id_product):
  try:
    conn = sqlite3.connect("product.db")
    conn.text_factory = str
    cursor = conn.cursor()
    cursor.execute("DELETE FROM data WHERE id = '%s'" % id_product)
    conn.commit()
    conn.close()
...
def clear():
  ...
  win.txt_date_register.setText("")
  win.txt_hour_register.setText("")
