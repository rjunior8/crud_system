...
class DelClient(QWidget):
  ...
  def initUI(self):
    ...
    self.setWindowIcon(QIcon("edit_delete.png"))
    ...
    self.btn_update = QPushButton("Deletar", self)
    self.btn_update.move(175, 600)
    self.btn_update.clicked.connect(self.delete)
...
def delete_data(id_client):
  try:
    conn = sqlite3.connect("client.db")
    conn.text_factory = str
    cursor = conn.cursor()
    cursor.execute("DELETE FROM data WHERE id = '%s'" % id_client)
    conn.commit()
    conn.close()
