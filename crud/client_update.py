...
class UpdateClient(QWidget):
  ...
  def initUI(self):
    self.setWindowTitle("Atualização de Cliente")
    self.setWindowIcon(QIcon("agt_update_product.png"))
    ...
    self.txt_id = QLabel(self)
    self.txt_id.move(10, 320)
    ...
    self.btn_update.move(175, 600)
    self.btn_update.clicked.connect(self.update)

    self.btn_cancel = QPushButton("Cancelar", self)
    self.btn_cancel.move(95, 600)
  ...
  def update(self):
    try:
      id_client = self.txt_id.text()
      if id_client != "":
        return update_data(id_client)
      else:
        QMessageBox.warning(self, "Error!", "Nenhum cliente selecionado!")
    except Exception as e:
      print(e)
      QMessageBox.warning(self, "Error!", "Dados incorretos!")
...
def update_data(id_client):
  try:
    conn = sqlite3.connect("client.db")
    conn.text_factory = str
    cursor = conn.cursor()

    name = win.txt_name.text().title().replace(" Da ", " da ").replace(" Das ", " das ").\
               replace(" De ", " de ").replace(" Dos ", " dos ")
    cpf = win.txt_cpf.text().replace('.', '').replace('-', '')
    born = win.txt_born.text().replace('/', '')
    sex = win.txt_sex.currentText()
    city = win.txt_city.text().title().replace(" Da ", " da ").replace(" Das ", " das ").\
               replace(" De ", " de ").replace(" Dos ", " dos ").replace(" Do ", " do ")
    nbd = win.txt_nbd.text().title().replace(" Da ", " da ").replace(" Das ", " das ").\
              replace(" De ", " de ").replace(" Dos ", " dos ").replace(" Do ", " do ")
    uf = win.txt_uf.text().upper()
    tel = win.txt_tel.text().replace('(', '').replace(')', '').replace('-', '')
    cel = win.txt_cel.text().replace('(', '').replace(')', '').replace('.', '').replace('-', '')
    whatsapp = win.txt_whatsapp.text().replace('(', '').replace(')', '').replace('-', '').replace('.', '')
    email = win.txt_email.text()
    addr = win.txt_addr.text().title().replace(" Da ", " da ").replace(" Das ", " das ").\
               replace(" De ", " de ").replace(" Dos ", " dos ").replace(" Do ", " do ")
    number = win.txt_number.text()
    debit = win.txt_debit.text().replace("R$", '').replace('.', '')
    low = win.txt_low.text().replace("R$", '').replace('.', '')
    parcels = win.txt_parcels.text()
    service = win.txt_service.toPlainText()

    if not debit.__contains__(',') or not low.__contains__(','):
      QMessageBox.warning(win, "Error!", "Valor(es) deve conter centavos após vírgula!")
    else:
      debit = debit.replace(',', '.')
      low = low.replace(',', '.')
      cursor.execute("UPDATE data SET name = ?, cpf = ?, born = ?, sex = ?, city = ?, nbd = ?, uf = ?, tel = ?, "
                     "cel = ?, whatsapp = ?, email = ?, addr = ?, number = ?, debit = ?, low = ?, parcel = ?, "
                     "service = ? WHERE id = ?",
                     (name, cpf, born, sex, city, nbd, uf, tel, cel, whatsapp, email, addr, number, debit, low,
                     parcels, service, id_client))
    conn.commit()
    conn.close()
    QMessageBox.information(win, "Informação!", "Operação realizada com sucesso!")
    return updated_data(name)

  except Exception as e:
    print(e)
    QMessageBox.warning(win, "Error!", "Dados incorretos!")

def updated_data(name):
  try:
    conn = sqlite3.connect("client.db")
    conn.text_factory = str
    cursor = conn.cursor()
